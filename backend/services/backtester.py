...
class VMCBacktester:
    ...
    def run(self):
        ...
        for entry_bar, direction, sig_type in signals:
            ...
            trade = Trade(
                direction=direction, entry_bar=entry_bar, entry_price=entry_price,
                stop_loss=sl, take_profit=tp, signal_type=sig_type,
                wt1_at_entry=df["wt1"].iloc[entry_bar],
                wt2_at_entry=df["wt2"].iloc[entry_bar],
                rsimfi_at_entry=df["rsimfi"].iloc[entry_bar],
            )
    ...
    def recent_trades(self, n: int = 10):
        ...
        for t in recent:
            result.append({
                ...
                "result":          t.result,
                "entry_time":      str(self.df["timestamp"].iloc[t.entry_bar]) if t.entry_bar < len(self.df) else "",
                "exit_time":       str(self.df["timestamp"].iloc[t.exit_bar])  if t.exit_bar  < len(self.df) else "",
                "wt2_at_entry":    round(t.wt2_at_entry, 2),
                "rsimfi_at_entry": round(t.rsimfi_at_entry, 2),
            })
        return result